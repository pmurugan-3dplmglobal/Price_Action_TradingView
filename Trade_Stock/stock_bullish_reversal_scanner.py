        ...
        for f in as_completed(futures):
            ...
            for name, scanner_func in scanners:
                ...
                if result:
                    ...
                    logging.info(f"  -> MATCH: {symbol} | {result['Pattern']} | Entry: {entry_val:.2f} | SL: {result['SL']:.2f} | T1: {result['T1']:.2f} | RR: {result['RR']:.2f}")
...
def print_summary(results):
    ...
    if matches:
        ...
        for m in sorted(matches, key=lambda x: x.get("RR", 0), reverse=True):
            ...
            print(f"  {m['Symbol']:<12} {m['Pattern']:<20} {c_s} {sl_s} {t1_s} {t2_s} {rr:<8.2f}")
    print("=" * 80)
